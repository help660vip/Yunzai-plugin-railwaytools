...
| `#实时 <车次>` | 实时状态查询成功时 Puppeteer 图片 | 普通文字 | 从真实车次详情派生 |
...
| `#铁路百科 CR400AF` | 本地百科命中时 Puppeteer 图片 | 普通文字 | 独立百科数据文件 |
| `#铁路百科 线路 宣杭铁路` | 动态线路资料命中时 Puppeteer 图片 | 普通文字 | 复用真实线路查询与缓存层 |
| `#随机列车` | 随机真实车次查询成功时 Puppeteer 图片 | 普通文字 | 从候选车次和百科数据派生 |
